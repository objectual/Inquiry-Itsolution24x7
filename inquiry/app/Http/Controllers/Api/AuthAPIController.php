...
class AuthAPIController extends AppBaseController
{
    ...
    public function register(RegistrationAPIRequest $request)
    {
        ...
        try {
            $checkingUser = $this->userRepository->checkingUser($request->email);

            if ($checkingUser != null) {
                if ($checkingUser->details->is_verified == 0 && $checkingUser->details->user_by == 1) {
                    return $this->sendResponse($checkingUser->id, 'User Already Registered');
                }
                if ($checkingUser->details->is_verified == 1 && $checkingUser->details->user_by == 1) {
                    $register = 1;
                    return $this->sendResponse($register, 'User Already Registered');
                }
...
//            }
            //attach role to user....
            $this->userRepository->attachRole($user->id, [Role::ROLE_CUSTOMER]);
            $credentials = [
                'email'    => $request->email,
